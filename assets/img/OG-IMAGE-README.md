# Open Graph Image

此檔案為社群分享預覽圖（Open Graph Image）。

## 建議規格

- **格式**: PNG 或 JPEG
- **尺寸**: 1200 x 630 像素（Facebook/LinkedIn 推薦）
- **檔案大小**: < 8 MB
- **內容建議**:
  - 湛盧科技 Logo（使用 `透明-LOGO帶公司名.png` 或 `透明-橫式LOGO.png`）
  - 公司標語或主要服務說明
  - 保持簡潔、專業的設計
  - 使用品牌色系（#006D77, #00B4BE）

## 用途

當您的網站連結在社群媒體（Facebook、Twitter、LinkedIn 等）分享時，會顯示此圖片作為預覽圖。

## 如何建立

1. 使用設計工具（如 Figma、Canva、Adobe Photoshop）建立 1200x630 的圖片
2. 加入公司 Logo（建議使用 `assets/img/company-logos/透明-LOGO帶公司名.png`）
3. 加入標語文字（例如：「專業醫療資訊系統與資料視覺化解決方案」）
4. 匯出為 PNG 格式
5. 將檔案命名為 `og-image.png` 並放在 `assets/img/` 資料夾

## 替代方案

如果暫時沒有自訂的 OG 圖片，可以直接使用公司橫式 Logo：
將所有 HTML 檔案中的 `assets/img/og-image.png` 替換為 `assets/img/company-logos/透明-橫式LOGO.png`
